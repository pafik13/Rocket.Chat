...
Meteor.methods({
	createPrivateGroup(name, members, readOnly = false, customFields = {}, extraData = {}) {
		...
		return createRoom('p', name, Meteor.user() && Meteor.user().username, members, readOnly, { customFields, membersHidden: false, filesHidden: false, canMembersAddUser: false, ...extraData });
	},
});
