/* global Plugin: false */
const stripComments = Npm.require('strip-json-comments');

// Check the config and log errors
const checkConfig = function(config) { // jshint ignore:line
	check(config, {
		apn: Match.Optional({
			passphrase: String,
			cert: String,
...
const clone = function(name, config, result) {
	if (typeof config[name] !== 'undefined') {
		result[name] = config[name];
	}
};

const cloneCommon = function(config, result) {
...
const configStringify = function(config) {
...
Plugin.registerSourceHandler('push.json', function(compileStep) {
	...
	const configString = stripComments(compileStep.read().toString('utf8'));
	...
	try {
		...
		const cloneConfig = archConfig[compileStep.arch];
